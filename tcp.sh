...
# Fichier de sortie pour les résultats TCP
fichier_sortie_tcp="resultats_tcp.txt"

# Supprimer le fichier de sortie s'il existe déjà
rm -f "$fichier_sortie_tcp"
...
find "$repertoire" -type f -name "*.pcap" | while read fichier_pcap; do
    ...
    tshark -r "$fichier_pcap" -Y "tcp" -T fields -e ip.src -e ip.dst -e tcp.srcport -e tcp.dstport -V | while IFS=$'\t' read -r ip_src ip_dst tcp_src_port tcp_dst_port; do
        ligne="$ip_src $ip_dst TCP $tcp_src_port $tcp_dst_port"
...
echo "L'analyse TCP est terminée. Les résultats uniques ont été enregistrés dans $fichier_sortie_tcp."
